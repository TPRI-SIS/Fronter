...
<script>
  ...
  let caculate = function (ele,vm) {


    var maxColumn = ele.outerWidth()
    var maxRow = ele.outerHeight()


    //获取网格的个数
    var columnCount = vm.columnCount
    var rowCount = vm.rowCount
    ...
    var columnDefine =  vm.columnDefine.split(",");
    var rowDefine = vm.rowDefine.split(",");
    ...
    $.each(ele.children(), function (index, item) {
      ...
      ii.css(
        {
          "position": "absolute",
          "left": getLength(unitWidthArray, c) + "px",
          "top":  getLength(unitHeightArray, r) + "px",
